...
def classify0(inX,dataSet,labels,k):
    ...
    sqDiffMat = diffMat ** 2
    sqDistances = sqDiffMat.sum(axis=1) # axis=0是列相加 axis=1行相加
    distances = sqDistances ** 0.5
    sortedDistIndicies = distances.argsort() # 返回按升序排序后原来的索引
    classCount = {}
    for i in range(k):
        voteIlabel = labels[sortedDistIndicies[i]]
        classCount[voteIlabel] = classCount.get(voteIlabel,0) + 1 # get根据key获取值,没有就返回0
